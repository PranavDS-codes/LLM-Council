...
export function CouncilTimeline() {
    const { activePhase, isProcessing } = useCouncilStore();

    return (
        <div className="max-w-4xl mx-auto px-6 py-12 space-y-12 pb-32">
            {/* 1. Generators */}
            {(activePhase >= 1) && (
                <TimelineItem phase={1} activePhase={activePhase} isProcessing={isProcessing} title="The Council Speaks" description="Divergent generation from selected agents.">
                    <PhaseGenerator />
                </TimelineItem>
            )}
            ...
            {/* 2. Critic */}
            {(activePhase >= 2) && (
                <TimelineItem phase={2} activePhase={activePhase} isProcessing={isProcessing} title="Peer Review" description="Critique and scoring of initial proposals.">
                    <PhaseCritic />
                </TimelineItem>
            )}
            ...
            {/* 3. Architect */}
            {(activePhase >= 3) && (
                <TimelineItem phase={3} activePhase={activePhase} isProcessing={isProcessing} title="The Blueprint" description="Synthesis of best ideas into a structured plan.">
                    <PhaseArchitect />
                </TimelineItem>
            )}
            ...
            {/* 4. Finalizer */}
            {(activePhase >= 4) && (
                <TimelineItem phase={4} activePhase={activePhase} isProcessing={isProcessing} title="The Verdict" description="Final cohesive response drafting.">
                    <PhaseFinalizer />
                </TimelineItem>
            )}

            {/* End Message */}
            {activePhase === 4 && !isProcessing && (
                <div className="text-center py-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
...
function TimelineItem({
    ...
    if (activePhase > phaseIndex) {
        ...
        // If it's the last phase (4), check isProcessing
        if (phaseIndex === 4) {
