    ...
    <section className="mx-auto max-w-[1400px] space-y-6 py-4">
      ...
      <div className="space-y-5">
        ...
        {chat.messages.map((message) => message.role === 'user' ? (
          ...
          <article key={message.id} className="relative max-w-none py-3 pr-24 text-left">
            ...
            {message.content ? <div className="prose prose-sm max-w-none text-left text-[var(--text-main)] dark:prose-invert prose-headings:text-left prose-headings:text-[var(--text-main)] prose-p:text-justify prose-p:text-[var(--text-main)] prose-li:text-left prose-li:text-[var(--text-main)] prose-table:text-left"><ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown></div> : message.status === 'streaming' ? <p className="animate-pulse text-sm italic text-[var(--text-muted)]">Preparing a response...</p> : null}
